...
def make_puma_tract_map():
    ...
    tp_rel = rg.dataframe(
        'https://www2.census.gov/geo/docs/maps-data/data/rel/2010_Census_Tract_to_2010_PUMA.txt#target_format=csv')  # PUMA <-> Tracts relationships
...
def get_table_raceiter(table, state, sl, year=2018, release=5):
    """Get all of the race iterations for a census table for a given summary level.  """

    acs_frames = [rg.dataframe(f'census://{year}/{release}/{state}/{sl}/{table.upper()}{race}').drop(
        columns=['stusab', 'county', 'name'])
        for race in [''] + list(race_map.keys())[1:] ]
...
def build_acs(state, sl='puma', year=2018, release=5, add_puma=False, output_type='Int64'):
    ...
    acs = acs.apply(pd.to_numeric, errors='ignore').astype(output_type, errors='ignore')

    acs = pd.DataFrame(acs.to_dict()) # Strip all traces of the CensusDataFrame

    if sl == 'tract' and add_puma:
        acs = acs.join(puma_tract_df().set_index('tract'))
        acs['puma'] = acs.puma.astype(str)
